<template>
    <div class="p-4">
        ...
        <div class="assets-grid">
            <div class="asset-card" v-for="(item, index) in resourceList" :key="item.path" :title="item.img_name">
                ...
                <div class="wrap">
                    <div class="image-wrap">
                        ...
                        <div class="actions-layer" v-if="selector">
                            <el-button type="primary" @click="$emit('asset-selected', item)">Select</el-button>
                        </div>
                        <div class="actions-layer" v-else>
                            <span v-if="isSupported" class="copy-item" @click="copy(item.src)"
                                title="Copy file path to clipboard">
                                <DocumentCopy />
                            </span>
                            <span v-if="item.type == 'image' && settings.canEdit" class="preview-item"
                                @click="$emit('edit', item.id)" title="Click to preview image">
                                <Edit />
                            </span>
                            <span class="preview-item" @click="handleOpenMedia(item.src)"
                                :title="`Click to open ${item.type} in new tab`">
                                <SvgIcon :icon="mdiOpenInNew" />
                            </span>
                            <span v-if="settings.canDelete" class="delete-item" @click="$emit('delete', [item.id])"

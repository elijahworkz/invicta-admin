        ...
        <div class="asset-wrap" v-if="fieldValue">
            ...
            <div class="asset-details mr-auto">
                ...
                <template v-if="isImage">
                    {{ fieldValue.width }}x{{ fieldValue.height }} | {{ fieldValue.size ? formatBytes(fieldValue.size) :
                        'n/a' }} {{ fieldValue.alt ? fieldValue.alt : 'No Alt!' }}
                </template>
            </div>
            <ActionsDropdown v-if="assetActions.length" :actions="assetActions" :item="fieldValue"
                @selected="handleFieldAction" class="mr-3" />
            <span class="mr-3 action-icon" @click="fieldValue = null" v-if="!field.disabled">
                <SvgIcon :icon="mdiTrashCanOutline" :width="18" />
            </span>
        </div>
        <Uploader v-else-if="!field.disabled" type="browse" @upload-complete="updateAsset" @open-library="openLibrary"
            :single="true" :folder="data.props?.folder || null" />
...
const isImage = computed(() => {
    return fieldValue.value.hasOwnProperty('type') ? fieldValue.value.type == 'image' : true
})

<template>
    ...
    >
        <template #item="{ element, index }">
            <li>
                <div
                    class="item flex items-center justify-start border rounded bg-white mb-1 p-2"
                    :class="{ error: element.error }"
                >
                    <DragHandle
                        class="mr-2 text-gray-300 hover:text-gray-400"
                    />
                    <el-link underline="never" @click="handleEdit(element)">{{
                        element.label
                    }}</el-link>
                    <div class="ml-auto item-type">
                        {{
                            element.error
                                ? `${element.type} | Error`
                                : element.type

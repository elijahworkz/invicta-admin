<template>
		...
		<div class="el-card">
			<div class="flex items-center justify-start p-3">
				...
				<div class="ml-auto leading-none flex items-center">
					<el-link v-if="canResetFilters" @click="resourceIndex.clearFilters" :underline="false" type="danger" class="mr-2">Clear all filters</el-link>
					<Filters v-if="resourceIndex.static.filters" :resource-handle="handle" :filters="resourceIndex.static.filters" :active-filters="resourceIndex.data.activeFilters" />
				</div>
			</div>
	...
	<div v-else class="p-10 flex items-center justify-center"><Loading/></div>
...
// Handle filters
const canResetFilters = computed(() => !isEmpty(resourceIndex.data.activeFilters))
